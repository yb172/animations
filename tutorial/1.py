...
class Testing(Scene):
  def construct(self):
    circle = Circle()
    circle.move_to(ORIGIN+2*LEFT)
    text = TextMobject("Movement from left to right")
    text.set_color(RED)
    self.add(text)
    self.wait(2)
    self.play(ShowCreation(circle))
    self.play(ApplyMethod(circle.move_to, circle.get_center()+4*RIGHT))
    self.wait(1)
...
class Shapes(Scene):
    ...
    def construct(self):
        circle = Circle()
        square = Square()
        line=Line(np.array([3,2,0]),np.array([5,0,0]))
